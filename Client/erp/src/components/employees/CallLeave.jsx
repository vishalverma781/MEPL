...
const CallLeave = () => {
 ...
 return (
    <div className="w-full min-h-screen flex justify-center items-start p-10 sm:p-10">
            ...
            <table className="w-full border border-gray-300 rounded-xl overflow-hidden">
              <thead>
                <tr className="bg-gray-800 text-white text-sm sm:text-base">
                  ...
                  <th className="p-3 text-left hidden md:table-cell">From</th>
                  <th className="p-3 text-left hidden md:table-cell">To</th>
                  <th className="p-3 text-left hidden md:table-cell">No. of Days</th>
              ...
              <tbody>
                {currentLeaves.map((leave) => (
                  ...
                  >
                    <td className="py-3 px-2 sm:px-3 font-bold text-gray-800">{leave.fullName || leave.username}</td>
                    <td className="py-3 px-2 sm:px-3 hidden md:table-cell">{formatDate(leave.startDate)}</td>
                    <td className="py-3 px-2 sm:px-3 hidden md:table-cell">{formatDate(leave.endDate)}</td>
                    <td className="py-3 px-2 sm:px-3 hidden md:table-cell">{leave.numberOfDays}</td>
                    <td className="py-3 px-2 sm:px-3" title={leave.reason}>
                      {leave.reason.length > 35 ? leave.reason.substring(0, 35) + "..." : leave.reason}
                    </td>
                    <td className={`py-3 px-2 sm:px-3 font-semibold ${statusColor[leave.status]}`}>
                      {leave.status}
                    </td>
...
{editableLeave && (
  <div className="fixed inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-50 p-4">
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg sm:max-w-3xl p-6 sm:p-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-gray-800 text-sm sm:text-base font-medium">
        <p>
          ...
          <span className="font-extrabold text-gray-900">{editableLeave.fullName || editableLeave.username}</span>
        </p>
        <p>
          <span className="font-bold">Status:</span>{" "}
          <span className={statusColor[editableLeave.status]}>{editableLeave.status}</span>
        </p>
        <p>
          <span className="font-bold">Date From:</span> {formatDate(editableLeave.startDate)}
        </p>
        <p>
          <span className="font-bold">Date To:</span> {formatDate(editableLeave.endDate)}
        </p>
        <p>
          <span className="font-bold">No. of Days:</span> {editableLeave.numberOfDays}
        </p>
