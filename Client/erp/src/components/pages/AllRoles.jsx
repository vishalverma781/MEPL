  ...
  return (
    <div className="w-full min-h-screen flex justify-center items-start p-10">
      <div className="bg-white shadow-2xl rounded-2xl p-8 w-full max-w-7xl">
        <h2 className="text-3xl font-bold text-center mb-6 text-gray-900">All Roles</h2>
            ...
            <table className="w-full border border-gray-300 rounded-xl overflow-hidden">
              <thead>
                <tr className="bg-gray-800 text-white text-xl">
                  ...
                  <th className="p-5 text-left">Assign Role</th>
                  <th className="p-5 text-left">Assigned By</th>
              ...
              <tbody>
                {currentRoles.map((role, idx) => (
                  <tr key={idx} className="border-b border-gray-200 hover:bg-gray-100 transition text-lg">
                    <td className="p-5">{role.employeeId?.fullName || "N/A"}</td>
                    <td className="p-5">{role.employeeId?.username || "N/A"}</td>
                    <td className="p-5">{role.role}</td>
                    <td className="p-5">{role.assignBy?.username || "Admin"}</td>
                    <td className="p-5">{role.fromDate ? formatDate(role.fromDate) : "N/A"}</td>
      ...
      {selectedRole && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-50">
          <div className="bg-white p-10 rounded-2xl shadow-2xl w-[90%] max-w-lg">
            <h3 className="text-3xl font-extrabold mb-8 text-center text-gray-900">Role Details</h3>
            <div className="space-y-5 text-gray-800 text-xl font-medium">
              ...
              {/* Project & Plaza for specific roles */}
