            ...
            <table className="w-full border border-gray-300 rounded-xl overflow-hidden">
              <thead>
                <tr className="bg-gray-800 text-white text-base sm:text-lg">
                  ...
                  <th className="p-4 text-left hidden sm:table-cell">From</th>
                  <th className="p-4 text-left hidden sm:table-cell">To</th>
                  <th className="p-4 text-left">No. of Days</th>
                  <th className="p-4 text-left hidden sm:table-cell">Description</th>
              ...
              <tbody>
                {currentLeaves.map((leave) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-3 hidden sm:table-cell">{formatDate(leave.startDate)}</td>
                    <td className="py-4 px-3 hidden sm:table-cell">{formatDate(leave.endDate)}</td>
                    <td className="py-4 px-3">{leave.numberOfDays}</td>
                    <td className="py-4 px-3 hidden sm:table-cell" title={leave.reason}>
                      {leave.reason.length > 35
                        ? leave.reason.substring(0, 35) + "..."
                        : leave.reason}
                    </td>
                    <td
                      className={`py-4 px-3 font-semibold ${statusColor[leave.status]}`}
                    >
                      {leave.status}
                    </td>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl p-6 sm:p-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-gray-800 text-sm sm:text-base font-medium">
              ...
              <p><span className="font-bold">Date From:</span> {formatDate(editableLeave.startDate)}</p>
              <p><span className="font-bold">Date To:</span> {formatDate(editableLeave.endDate)}</p>
              <p><span className="font-bold">No. of Days:</span> {editableLeave.numberOfDays}</p>
