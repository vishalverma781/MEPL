...
const ManageAdmins = () => {
  ...
  <div className="flex-1 min-h-screen overflow-x-auto p-10 overflow-y-auto transition-all duration-300">
    ...
    {/* Top Message */}
    {message && (
      <div className={`fixed top-4 left-1/2 -translate-x-1/2 px-4 py-2 rounded-md shadow-lg text-white text-sm font-semibold ${
          ...
          <table className="w-full border border-gray-300 rounded-lg overflow-hidden">
            <thead>
              <tr className="bg-gray-800 text-white text-sm">
                ...
                <th className="p-3 text-left hidden xl:table-cell">Position</th>
            ...
            <tbody>
              {currentAdmins.map((admin) => (
                <tr key={admin._id} className="border-b border-gray-200 hover:bg-gray-100 transition text-sm">
                  ...
                  <td className="py-3 px-3 hidden lg:table-cell">{admin.address?.city || "N/A"}</td>
                  <td className="py-3 px-3 hidden lg:table-cell">{admin.address?.state || "N/A"}</td>
                  <td className="py-3 px-3 hidden xl:table-cell">{admin.position || "N/A"}</td>
                  <td className="py-3 px-3 hidden xl:table-cell">{admin.address?.homeAddress || "N/A"}</td>

                  <td className="py-3 px-3 text-center">
                    <button onClick={() => setSelectedAdmin(admin)} className="text-green-600 hover:text-green-800">
                      <FaEye size={18} />
                    </button>
                  </td>
                  ...
                  <td className="py-3 px-3 text-center">
                    <button onClick={() => handleDelete(admin._id)} className="text-red-600 hover:text-red-800">
                      <FaTrash size={18} />
                    </button>
                  </td>
                </tr>
    ...
    {editAdmin && (
      <div className="fixed inset-0 flex items-center justify-center bg-black/30 backdrop-blur-sm z-50 p-3">
        <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-md">
          ...
          <form className="space-y-3 text-gray-700 text-sm font-medium">
            <div>
            ...
            <div>
              <label className="block mb-1 font-semibold">Home Address</label>
              <input type="text" name="homeAddress" value={formData.address.homeAddress} onChange={handleChange}
                className="w-full px-2 py-1 border rounded text-sm" />
            </div>
